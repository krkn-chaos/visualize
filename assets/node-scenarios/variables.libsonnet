{
  datasourceVariable(name, label, query, regex='', hide=0):: {
    current: { selected: false, text: 'Chaos ES - Telemetry', value: '${Datasource}' },
    hide: hide, includeAll: false, label: label, multi: false, name: name,
    options: [], query: query, queryValue: '', refresh: 1, regex: regex,
  ...
  customVariable(name, label, query, defaultValue, hide=0):: {
    ...
    query: query, skipUrlSync: false, type: 'custom',
  },

  queryVariable(name, label, query, datasourceUid, includeAll=false, multi=false, hide=0, definition='', regex=''):: {
    current: { selected: false, text: '', value: '' },
    datasource: { type: 'grafana-opensearch-datasource', uid: datasourceUid },
    definition: definition, hide: hide, includeAll: includeAll, label: label,
    multi: multi, name: name, options: [], query: query, refresh: 1, regex: regex,
    skipUrlSync: false, sort: 0, type: 'query',
  },

  getAllVariables()::
    local scenario_path = '/tmp/node_scenario.yaml';
    local scenarioFilter = 'scenarios.scenario: ' + scenario_path;
    ...
    local runUuidFilters = [
      ...
      'scenarios.parameters.node_scenarios.label_selector.keyword: $node_selector',
      'tag.keyword: $tag',
    ...
    [
      ...
      self.queryVariable('node_count', 'node_count', '{"find": "terms", "field": "node_summary_infos.count", "query": "cloud_infrastructure.keyword: $platform AND ' + scenarioFilter + ' AND cloud_type.keyword: $cloud_type"}', '${Datasource}'),
      self.queryVariable('major_version', 'major_version', '{"find": "terms", "field": "major_version.keyword", "query": "cloud_infrastructure.keyword: $platform AND network_plugins.keyword: $networkType AND ' + scenarioFilter + ' AND cloud_type.keyword: $cloud_type"}', '${Datasource}', true, true),
      self.queryVariable('node_selector', 'node_selector', '{"find": "terms", "field": "scenarios.parameters.node_scenarios.label_selector.keyword", "query": "cloud_infrastructure.keyword: $platform AND network_plugins.keyword: $networkType AND ' + scenarioFilter + ' AND cloud_type.keyword: $cloud_type"}', '${Datasource}', true, true),
