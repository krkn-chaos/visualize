...

{
  getAllPanels():: [
    ...
    chaos_panels.scenarioUuidDetails('Time Scenario UUID Details', queries.timeScenarioUuidDetails),
    chaos_panels.alertsRow(),
    chaos_panels.scenarioAlerts(),
    // self.timeMetricsRow(),
    timeseriesPanel(2, 'Clock Skew Duration', 'Duration for which the clock skew was applied to the target node.', queries.clockSkewDuration, 's', 0, 21, 12),
    timeseriesPanel(3, 'NTP Recovery Time', 'Time taken for NTP to re-synchronize the clock after the chaos scenario.', queries.ntpRecoveryTime, 's', 12, 21, 12),
