{
  ...
  getAllVariables():: [
    ...
    self.queryVariable('node_count', 'node_count', '{"find": "terms", "field": "node_summary_infos.count",  "query": "cloud_infrastructure.keyword: $platform AND scenarios.scenario: /tmp/app_outages.yaml AND cloud_type.keyword: $cloud_type"}', '${Datasource}'),
    self.queryVariable('major_version', 'major_version', '{"find": "terms", "field": "major_version.keyword",  "query": "cloud_infrastructure.keyword: $platform AND network_plugins.keyword: $networkType AND scenarios.scenario: /tmp/app_outages.yaml AND cloud_type.keyword: $cloud_type" }', '${Datasource}', true, true),
    self.queryVariable('run_uuid', 'run_uuid', '{"find": "terms", "field": "run_uuid.keyword", "query": "cloud_infrastructure.keyword: $platform AND network_plugins.keyword: $networkType AND scenarios.scenario: /tmp/app_outages.yaml AND major_version.keyword: $major_version AND cloud_type.keyword: $cloud_type AND node_summary_infos.count: $node_count"}', '${Datasource}', true, true),
  ],
}
