{
  ...
  getAllVariables():: [
    ...
    self.queryVariable('run_uuid', 'run_uuid', '{"find": "terms", "field": "run_uuid.keyword", "query": "cloud_infrastructure.keyword: $platform AND network_plugins.keyword: $networkType AND scenarios.scenario: /tmp/pod_scenario.yaml AND major_version.keyword: $major_version AND cloud_type.keyword: $cloud_type AND node_summary_infos.count: $node_count AND scenarios.parameters.config.namespace_pattern.keyword: $namespace"}', '${Datasource}', true, true),
  ],
}
